...
public class PacketHelper {
	...
	public static void addStartedQuestInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
		...
		for (MapleQuestStatus q : started) {
			...
			if (q.hasMobKills()) {
				...
				mplew.writeMapleAsciiString(q.getCustomData() == null ? "Hellov1" : q.getCustomData());
			}
		}
	...
	public static void addNXQuestInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
		mplew.writeShort(0); // changed from mplew.writeShort(1)
	}
	...
	public static void addMoneyInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
		...
		mplew.writeInt(0); // size for (int + int + int + long)
        ...
        mplew.writeInt(0);
        mplew.write(0); // boolean for (9 ints + 3 longs)
        mplew.write(0); // size
        mplew.write(0); // size 
		...
		for (Item item : equipped) {
			if ((item.getPosition() < 0) && (item.getPosition() > -100)) {
				...
				//if (equip.getSoulEnchanter() != 0) { // Apply soul buff on login
				//	SkillFactory.getSkill(SkillConstants.soulSkill(equip.getSoulName())).getEffect(1).applySoulWeapon(chr, (short) chr.soul, false);
		...
		mplew.writeShort(0);//ab transformed items // 6
		for (Item item : equipped) {
		...
		mplew.writeShort(0);//zero female cs items // 8
		for (Item item : equipped) {
		...
		mplew.writeShort(0);//kanna haku wep // 11
		for (Item item : equipped) {
		...
		mplew.write(0); // p2 28 bytes betewwn equip and etc
...
public static void addCharStats(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
	...
	public static void addEquipBonusStats(MaplePacketLittleEndianWriter mplew, Equip equip, boolean hasUniqueId) {
		...
		mplew.writeLong(0);
		mplew.writeLong(0);
		mplew.writeZeroBytes(6); // unkn v171.1	
	}
	...
	public static void addCharacterInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {                
		long mask = -1L; // FF FF FF FF FF FF DF FF v146
		...
		if (GameConstants.isAngelicBuster(chr.getJob())) {
			...
			mplew.writeLong(getTime(System.currentTimeMillis()));
			mplew.writeZeroBytes(14); // Was 17 Before
		}
		...
		mplew.writeShort(2);
		mplew.writeInt(9);
		mplew.writeMapleAsciiString("check1=0;cDate=16/04/21");
                mplew.writeInt(6);
		mplew.writeMapleAsciiString("enter=201606");
		/////////////////////////////////////////////////
	...
	public static void addCoreAura(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
		MapleCoreAura aura = new MapleCoreAura(0, 0);
