...
public class CField {
	...
	public static final byte[] Nexon_IP2 = new byte[]{(byte) 8, (byte) 31, (byte) 99, (byte) 165};//current 2nd ip
	public static final byte[] Unknown_IP = new byte[]{(byte) 8, (byte) 31, (byte) 99, (byte) 133}; //v171
	public static byte[] getPacketFromHexString(String hex) {
		return HexTool.getByteArrayFromHexString(hex);
	}

	public static byte[] getServerIP(MapleClient c, int port, int clientId, int channel) {
		...
		mplew.write(Unknown_IP);
		mplew.writeShort(8785); // Scania Port
	...
	public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
		...
		mplew.writeInt(chr.getId()); // 4
		mplew.write(chr.getLevel()); // 5
                mplew.writeShort(7); // 7
		mplew.writeMapleAsciiString(chr.getName()); // any name.. private = 7
		MapleQuestStatus ultExplorer = chr.getQuestNoAdd(MapleQuest.getInstance(111111));
		if ((ultExplorer != null) && (ultExplorer.getCustomData() != null)) {
			mplew.writeMapleAsciiString(ultExplorer.getCustomData());
		} else {
			mplew.writeMapleAsciiString("Hi this is v174 LOL!");
		}
		if (chr.getGuildId() <= 0) {
			mplew.writeZeroBytes(8);
		} else {
		...
		mplew.writeZeroBytes(64); //v143 20->38 ty hawt 106

		int CHAR_MAGIC_SPAWN = Randomizer.nextInt();
		mplew.write(1); // 107
		mplew.writeInt(CHAR_MAGIC_SPAWN); // 111
		mplew.writeZeroBytes(8); //v143 10->8 119
		mplew.write(1); // 120
		mplew.writeInt(CHAR_MAGIC_SPAWN); // 124
		mplew.writeZeroBytes(10); // 134
		mplew.write(1); // 135
		mplew.writeInt(CHAR_MAGIC_SPAWN); // 139
		mplew.writeShort(0); // 141
		...
		if (buffSrc > 0) {
			...
			if ((GameConstants.getMountItem(buffSrc, chr) == 0) && (c_mount != null) && (chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -119) != null)) {
				mplew.writeInt(c_mount.getItemId()); // 145
				...
				mplew.writeInt(mount.getItemId()); // 149
			} else {
				mplew.writeInt(GameConstants.getMountItem(buffSrc, chr)); // 153
			}
			mplew.writeInt(buffSrc); // 157
		} else {
			mplew.writeLong(0L); // 163
		}
		mplew.write(1); // 164
		mplew.writeInt(CHAR_MAGIC_SPAWN); // 168
		mplew.writeLong(0L); // 172
		mplew.write(1); // 173
		mplew.writeInt(CHAR_MAGIC_SPAWN); // 177
		mplew.write(GameConstants.isZero(chr.getJob()) || GameConstants.isEvan(chr.getJob()) ? 1 : 0); //Shows the dragon in inventory it seems
		mplew.writeZeroBytes(14);// There must be something in here... But what?... // 191
		mplew.write(1); // 192
		...
		mplew.writeShort(0); //new v142->v143
		mplew.writeInt(1); //new v142->v143
	...
	public static byte[] closeRangeAttack(int cid, int tbyte, int skill, int level, int display, byte speed, List<AttackPair> damage, boolean energy, int lvl, byte mastery, byte unk, int charge, MapleCharacter chr) {
        return addAttackInfo(energy ? 4 : 0, cid, tbyte, skill, level, display, speed, damage, lvl, mastery, unk, 0, null, 0, chr);
    }

    public static byte[] rangedAttack(int cid, byte tbyte, int skill, int level, int display, byte speed, int itemid, List<AttackPair> damage, Point pos, int lvl, byte mastery, byte unk, MapleCharacter chr) {
        return addAttackInfo(1, cid, tbyte, skill, level, display, speed, damage, lvl, mastery, unk, itemid, pos, 0, chr);
    }

    public static byte[] strafeAttack(int cid, byte tbyte, int skill, int level, int display, byte speed, int itemid, List<AttackPair> damage, Point pos, int lvl, byte mastery, byte unk, int ultLevel, MapleCharacter chr) {
        return addAttackInfo(2, cid, tbyte, skill, level, display, speed, damage, lvl, mastery, unk, itemid, pos, ultLevel, chr);
    }

    public static byte[] magicAttack(int cid, int tbyte, int skill, int level, int display, byte speed, List<AttackPair> damage, int charge, int lvl, byte unk, MapleCharacter chr) {
        return addAttackInfo(3, cid, tbyte, skill, level, display, speed, damage, lvl, (byte) 0, unk, charge, null, 0, chr);
    }

    public static byte[] addAttackInfo(int type, int cid, int tbyte, int skill, int level, int display, byte speed, List<AttackPair> damage, int lvl, byte mastery, byte unk, int charge, Point pos, int ultLevel, MapleCharacter chr) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        if (GameConstants.isZero(skill / 10000)) {
            int v13 = 0;
            mplew.write(v13);
            if (v13 != 0) {
                mplew.writeShort(0);// v14
                mplew.writeShort(0);// v13
            }
        }
        
        if (skill == 101110104) {
            return mplew.getPacket(); // using IDA :/
        }
        ...
        // maybe the ult level lol :/
        if ((type == 1 || type == 2) && (GameConstants.isZero(skill))) {
            mplew.write(0);
        }
        
        if (skill == 40021185 || skill == 42001006 || skill == 80011067) {
            mplew.write(0);
        }
        ...
        mplew.writeInt(0); // ?
        if ((unk & 2) != 0) {
          //  mplew.writeInt(chr.getStatForBuff(MapleBuffStat.SkillCancel).getSourceId());
          //  mplew.writeInt(chr.getStatForBuff(MapleBuffStat.SkillCancel).getLevel());
        }
        ...
        for (AttackPair oned : damage) {
            if (oned.attack != null) {
                ...
                mplew.write(0);
                mplew.writeShort(0);
                if (skill == 42111002 || skill == 80011050) {
        ...
        if (skill == 2321001 || skill == 2221052 || skill == 11121052 || skill == 12121054) {
            mplew.writeInt(0);
        } else if (skill == 4221052 || skill == 65121052 || skill == 80001431 || skill == 100001283 || skill == 80001429 || skill == 13121052 || skill == 14121052 || skill == 15121052 || skill == 101000202 || skill == 101000102) {
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        ...
        if (skill == 5321000 || skill == 5311001 || skill == 5321001 || skill == 5011002 || skill == 5311002 || skill == 5221013 || skill == 5221017 || skill == 3120019 || skill == 3121015 || skill == 4121017) {
            mplew.writePos(pos);
        }
        return mplew.getPacket();
    }
...
public static byte[] removeNPCController(int objectid) {
		...
		public static byte[] getNPCTalk(int npc, byte msgType, String talk, String endBytes, byte type, int diffNPC) {
            ...
            mplew.writeInt(0);
            mplew.write(0);
            mplew.write(msgType); // msgtype
            mplew.write(4);
            mplew.write(0);
            mplew.writeInt(npc);
            // idk where type is and I have no clue where this conditional statement belongs
            if ((type & 0x04) != 0) {//This one might've change
